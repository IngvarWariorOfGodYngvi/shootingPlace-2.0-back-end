...
public class ListOfErasedXlsxGenerator {
    ...
    public XlsxGenerationResult generate(LocalDate firstDate, LocalDate secondDate) throws IOException {
        ...
        List<MemberDTO> collect = memberRepository.findAllByErasedTrue().stream().filter(f -> f.getErasedEntity() != null).filter(f -> f.getErasedEntity().getDate().isAfter(firstDate.minusDays(1))).filter(f -> f.getErasedEntity().getDate().isBefore(secondDate.plusDays(1))).map(Mapping::map2DTO).sorted(Comparator.comparing(MemberDTO::getJoinDate).thenComparing(MemberDTO::getSecondName).thenComparing(MemberDTO::getFirstName)).toList();
